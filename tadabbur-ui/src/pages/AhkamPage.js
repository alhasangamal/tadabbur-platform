...
import { Search, Gavel, BookOpen, Layers, Filter, CheckCircle2 } from "lucide-react";
...
export default function AhkamPage() {
  ...
  const categories = useMemo(() => {
    const allCats = ahkamData.flatMap((item) => item.category || []);
  ...
  const filteredData = useMemo(() => {
    return ahkamData.filter((item) => {
      ...
      const matchesSearch = 
        item.short_text.toLowerCase().includes(searchLower) ||
        item.ayah_text.toLowerCase().includes(searchLower) ||
        item.explanation.toLowerCase().includes(searchLower) ||
        item.surah_name.includes(searchQuery);
  ...
  // Stats
  const stats = useMemo(() => [
    { label: "حكم شرعي", value: ahkamData.length, icon: Gavel },
    { label: "سورة", value: new Set(ahkamData.map(d => d.surah_name)).size, icon: BookOpen },
    { label: "تصنيف فرعي", value: categories.length - 1, icon: Layers },
  ], [categories]);
    ...
    <div className="space-y-12 pb-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade">
      ...
      <section className="text-center space-y-4">
        ...
        <p className="text-gray-600 dark:text-gray-400 max-w-2xl mx-auto text-lg">
          استعراض شامل للآيات التي تضمنت أحكاماً فقهية في العقائد والعبادات والمعاملات، مع شرح ميسر لكل آية.
        </p>
      </section>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {stats.map((stat, idx) => (
          <div key={idx} className="bg-white dark:bg-gray-800 border border-emerald-100 dark:border-gray-700 rounded-2xl p-6 shadow-sm flex items-center justify-between">
      ...
      <DataFilterHeader 
        ...
        setSearchTerm={setSearchQuery} // Note: The component uses setSearchTerm prop but I'll check my component definition
        categories={categories}
        selectedCategory={selectedCategory}
        setSelectedCategory={setSelectedCategory}
        placeholder="ابحث في الأحكام الفقهية أو السور..."
        isRtl={isRtl}
      />
          ...
          {filteredData.map((item, idx) => (
            ...
            >
              ...
              <div className="bg-[#fcfbf7] dark:bg-gray-900 px-6 py-4 flex justify-between items-center border-b border-emerald-50 dark:border-gray-700">
                ...
                <div className="text-emerald-200 group-hover:text-emerald-500 transition-colors hidden sm:block">
              ...
              <div className="p-8 space-y-6 flex-1">
                ...
                <div className="space-y-2">
                  <h4 className="text-sm font-bold text-gray-400 dark:text-gray-500 uppercase tracking-tighter flex items-center gap-2">
                    <BookOpen className="w-4 h-4" /> التفسير والبيان
                  </h4>
      ...
      {filteredData.length === 0 && (
        <div className="py-20 text-center space-y-4">
          ...
          <h3 className="text-xl font-medium text-gray-500">لم يتم العثور على نتائج تطابق بحثك</h3>
          <button 
