  ...
  const filteredData = useMemo(() => {
    return nasikhData.filter((item) => {
      ...
      const matchesSearch = 
        ...
        (item.mansukh_text && item.mansukh_text.toLowerCase().includes(searchLower)) ||
        (item.nasikh_text && item.nasikh_text.toLowerCase().includes(searchLower)) ||
        item.surah_name.includes(searchTerm);
  ...
  return (
    <div className="space-y-12 pb-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade">
      ...
      <DataFilterHeader 
        ...
        placeholder="ابحث في الناسخ والمنسوخ..."
        isRtl={isRtl}
      />
