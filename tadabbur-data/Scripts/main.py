...
def get_verses_by_keys(keys: str = Query(..., description="Comma-separated ayah keys like 2:30,7:142")):
    """
    Accepts verse references like '2:30, 7:142, 28:34'
    Returns the full verse text (uthmani) and surah name for each.
    """
...
def get_entity_neighbors(slug: str):
    query = """
    MATCH (e:Entity {slug:$slug})-[r:RELATES]->(x:Entity)
    RETURN
        ...
        r.evidence_notes AS evidence_notes
    UNION
    MATCH (x:Entity)-[r:RELATES]->(e:Entity {slug:$slug})
    RETURN
...
@app.get("/verses/by-keys")
def get_verses_by_keys(keys: str = Query(...)):
    """Fetch verse texts by comma-separated ayah_keys like '2:30, 7:11, 15:26'"""
    key_list = [k.strip() for k in keys.split(",") if k.strip()]
    if not key_list:
        return {"verses": []}
