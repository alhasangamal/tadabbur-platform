"""
سكريبت جمع بيانات الأدعية القرآنية من موقع i-quran.com/duas
...
URL         = "https://i-quran.com/duas"
OUTPUT_JSON = "الأدعية.json"
OUTPUT_TEXT = "الأدعية.txt"
...
def scrape_item(card, page):
    ...
    item["الدعاء"] = el.inner_text().strip() if el else ""
...
def scrape_all(page):
    ...
    print(f"  📋 {len(cards)} دعاء\n")

    for i, card in enumerate(cards, 1):
        try:
            data = scrape_item(card, page)
            print(f"  [{i:>3}] {data.get('الدعاء','')[:55]}")
...
def format_text(results):
    lines = ["🤲 الأدعية القرآنية", "=" * 70, ""]
    for item in results:
        lines.append(f"الدعاء  : {item.get('الدعاء','')}")
...
def main():
    print("=" * 60)
    print("[Duas] Quranic Duas — i-quran.com/duas")
    print("=" * 60)
    ...
    print(f"\n{'='*60}")
    print(f"✅ تم جمع {len(results)} دعاء")
    if not results:
