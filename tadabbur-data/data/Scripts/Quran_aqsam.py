"""
سكريبت جمع بيانات الأقسام القرآنية من موقع i-quran.com/aqsam
...
URL         = "https://i-quran.com/aqsam"
OUTPUT_JSON = "الأقسام.json"
OUTPUT_TEXT = "الأقسام.txt"
...
def scrape_item(card, page):
    ...
    item["القسم"] = el.inner_text().strip() if el else ""
...
def scrape_all(page):
    ...
    print(f"  📋 {len(cards)} قسم\n")

    for i, card in enumerate(cards, 1):
        try:
            data = scrape_item(card, page)
            print(f"  [{i:>3}] {data.get('القسم','')[:55]}")
...
def format_text(results):
    lines = ["📜 الأقسام القرآنية", "=" * 70, ""]
    for item in results:
        lines.append(f"القسم   : {item.get('القسم','')}")
...
def main():
    print("=" * 60)
    print("[Aqsam] Quranic Oaths — i-quran.com/aqsam")
    print("=" * 60)
    ...
    print(f"\n{'='*60}")
    print(f"✅ تم جمع {len(results)} قسم")
    if not results:
