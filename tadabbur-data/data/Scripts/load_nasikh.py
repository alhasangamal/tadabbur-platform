...
def extract_card_record(card):
    """
    نحاول استخراج الحقول بشكل مرن لأن أسماء الـ classes قد تختلف قليلًا.
    """
    ref_full = ""
    title_ar = ""
    note_ar = ""
    ...
    for selector in ref_selectors:
        ref_full = safe_text(card.locator(selector).first)
        if ref_full:
            break
    ...
    for selector in title_selectors:
        title_ar = safe_text(card.locator(selector).first)
        if title_ar:
            break
    ...
    for selector in note_selectors:
        note_ar = safe_text(card.locator(selector).first)
        if note_ar:
            break

    # نصوص الآيات داخل البطاقة
    verse_selectors = [
        ".da-text",
        ".ns-text",
        "[class*='text']",
    ...
    # توزيع النصوص:
    # غالبًا الناسخ والمنسوخ فيه آيتان، لذلك:
    # أول نص = المنسوخ، ثاني نص = الناسخ
    mansukh_text = verse_texts[0] if len(verse_texts) > 0 else ""
    nasikh_text = verse_texts[1] if len(verse_texts) > 1 else ""

    record = {
        "surah_name": surah_name,
        "ayah_number": ayah_number,
        "title": title_ar,
        "mansukh_text": mansukh_text,
        "nasikh_text": nasikh_text,
        "category": categories,
        "explanation": note_ar,
    }

    # لو لم نجد أي بيانات مهمة نرجع None
    if not any([
        record["surah_name"],
        record["title"],
        record["mansukh_text"],
...
def scrape_nasikh():
    ...
    with sync_playwright() as p:
        ...
        # نجرب أكثر من selector محتمل للبطاقات
        card_selectors = [
            ".nsk-card",
            ".nasikh-card",
            ".abg-card",
        ...
        for i in range(count):
            ...
            try:
                ...
                fingerprint = (
                    ...
                    record["ayah_number"],
                    record["title"][:80],
                    record["mansukh_text"][:60],
                    record["nasikh_text"][:60],
                )
...
if __name__ == "__main__":
    ...
    with open("nasikh_data.json", "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False, indent=2)

    print(f"Saved {len(data)} records to nasikh_data.json")